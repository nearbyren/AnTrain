package com.testrecycling.toolsapp.dao
...
@Dao interface ResFlowDao {
    ...
    @Query("select * from ResEntity ORDER BY ROWID DESC LIMIT 1") fun queryResEntityMax(): ResEntity

    @Query("select * from ResEntity where cmd = :cmd and id = (select MAX(id) from ResEntity where cmd  = :cmd)")
