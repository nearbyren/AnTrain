...
object TaskLightsScheduler {

    // 任务标识前缀
    private const val DAILY_TASK_PREFIX = "lights_daily_task_"
    private const val IMMEDIATE_TASK_PREFIX = "lights_immediate_task_"
     ...
     */
    @RequiresApi(Build.VERSION_CODES.O)
    fun triggerImmediately(context: Context, taskName: String? = null, executionTime: String? = null) {
        val workManager = WorkManager.getInstance(context)

        // 生成唯一的任务名称
        val fullTaskName = buildTaskName(IMMEDIATE_TASK_PREFIX, executionTime ?: "now", taskName)

        val immediateRequest = buildImmediateRequest(executionTime)
    ...
    private fun buildDailyRequest(time: String): PeriodicWorkRequest {
        ...
        return PeriodicWorkRequestBuilder<DailyLightsWorker>(24, TimeUnit.HOURS)
            .setInitialDelay(calculateInitialDelay(time)) // 初始延迟到指定时间
            .setInputData(inputData) // 设置输入数据
            .addTag("lights_daily_task")
            .build()
    }
    // 获取当前时间字符串
    @RequiresApi(Build.VERSION_CODES.O)
    ...
    // 构建立即请求
    @RequiresApi(Build.VERSION_CODES.O) private fun buildImmediateRequest(time: String? = null): OneTimeWorkRequest {
        val currentTime = if (time != null) time else getCurrentTime()

        val inputData = workDataOf(
            "scheduled_time" to currentTime,
            "task_type" to "immediate",
            "actual_execution_time" to System.currentTimeMillis()
        )
        return OneTimeWorkRequestBuilder<DailyLightsWorker>()
            .setInputData(inputData) // 设置输入数据
            .addTag("lights_immediate_task")
            .build()
    }
