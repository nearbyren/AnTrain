package com.testrecycling.toolsapp.socket


import com.testrecycling.toolsapp.utils.AppUtils
import com.testrecycling.toolsapp.utils.Loge
import com.testrecycling.toolsapp.db.DatabaseManager
import com.testrecycling.toolsapp.utils.CmdValue
import com.testrecycling.toolsapp.utils.JsonBuilder
import com.testrecycling.toolsapp.utils.SPreUtil
...
class SocketClient(
     ...
     */
    suspend fun start() {
        Loge.e("调试socket SocketClient start running $running $clientScope")
        if (running) return
        running = true
     ...
     */
    suspend fun stop() {
        Loge.e("调试socket SocketClient stop ")
        running = false
        try {
            clientScope.coroutineContext.job.cancelAndJoin()
        } catch (e: CancellationException) {
            Loge.e("调试socket SocketClient stop ${e.message}")
        }
        closeSocketQuietly()
    }
...
//        Loge.e("调试socket SocketClient send ByteArray  ${ByteUtils.toHexString(data)}")
...
//        Loge.e("调试socket SocketClient sendText  $text")
...
//        Loge.e("调试socket SocketClient enqueueSend  ${ByteUtils.toHexString(data)}")
    ...
    private suspend fun runMainLoop() {
        var attempt = 0
        Loge.e("调试socket SocketClient runMainLoop $running ${clientScope.isActive}")
        while (running && clientScope.isActive) {
            try {
                _state.value = ConnectionState.CONNECTING
                _state2.emit(ConnectionState.CONNECTING)
                Loge.e("调试socket SocketClient runMainLoop 连接中")
                connectAndServe()
                attempt = 0 // reset backoff after successful session
            } catch (e: CancellationException) {
                Loge.e("调试socket SocketClient runMainLoop catch1 ${e.message} running $running")
                break
            } catch (e: Exception) {
                // Swallow and backoff
                Loge.e("调试socket SocketClient runMainLoop catch2 ${e.message} running $running")
            } finally {
                Loge.e("调试socket SocketClient runMainLoop finally running $running")
                closeSocketQuietly()
                if (!running) break
            ...
            Loge.e("调试socket SocketClient runMainLoop 重连接延迟 $delayMs")
            delay(delayMs)
        }
    }
    ...
    private fun computeReconnectDelay(attempt: Int): Long {
        Loge.e("调试socket SocketClient computeReconnectDelay attempt $attempt")
        val base =
    ...
    private suspend fun connectAndServe() {
        Loge.e("调试socket SocketClient connectAndServe")
        val s = Socket()
        s.tcpNoDelay = true
...
//        val monitor = clientScope.launch { heartbeatAndIdleMonitor() }
        Loge.e("调试socket SocketClient connectAndServe 已连接")
...
//        monitor.cancel()
    ...
    private suspend fun readLoop(input: BufferedInputStream) {
        Loge.e("调试socket SocketClient readLoop ")
...
//                Loge.e("调试socket SocketClient readLoop ${ByteUtils.toHexString(frame)}")
                _incoming.emit(frame)
            } catch (e: IOException) {
                Loge.e("调试socket SocketClient readLoop catch ${e.message}")
                break
            }
        }
    }
    ...
    private suspend fun writeLoopByte(output: BufferedOutputStream) {
        Loge.e("调试socket SocketClient writeLoop running $running | isActive ${clientScope.isActive}")
        while (running && clientScope.isActive) {
            try {
                val data = sendQueueByte.receive()
//                Loge.e("调试socket SocketClient writeLoopByte byte：${ByteUtils.toHexString(data)}")
                output.write(data)
                ...
                Loge.e("调试socket SocketClient writeLoop catch1 ${e.message}")
                break
            } catch (e: IOException) {
                Loge.e("调试socket SocketClient writeLoop catch2 ${e.message}")
                break
            }
        }
    }
...
//       Loge.e("调试socket SocketClient heartbeatAndIdleMonitor $running | ${clientScope.isActive}")
        val hasHeartbeat =
                config.heartbeatIntervalMillis > 0 /*&& config.heartbeatPayload.isNotEmpty()*/
        while (running && clientScope.isActive) {
            val now = System.currentTimeMillis()
//            Loge.e("调试socket SocketClient heartbeatAndIdleMonitor 分钟：${config.idleTimeoutMillis} | 当前毫秒：$lastReceivedAtMillis | 当前-最后：${now - lastReceivedAtMillis}")
            if (config.idleTimeoutMillis > 0 && now - lastReceivedAtMillis > config.idleTimeoutMillis) {
                // Force reconnect by closing the socket
                Loge.e("调试socket SocketClient heartbeatAndIdleMonitor closeSocketQuietly")
                closeSocketQuietly()
                return
            }
//            Loge.e("调试socket SocketClient heartbeatAndIdleMonitor $hasHeartbeat")
            if (hasHeartbeat) {
                try {
//                    Loge.e("调试socket SocketClient heartbeatAndIdleMonitor trySend")
                    val stateList = DatabaseManager.queryStateList(AppUtils.getContext())
//                    Loge.e("调试socket SocketClient stateList：${stateList.size}")
                    val setSignal = SPreUtil[AppUtils.getContext(), "setSignal", 19] as Int
                    // 构建JSON对象
...
//                    Loge.e("调试socket SocketClient 发送心跳数据：$jsonObject")
                    val byteArray = JsonBuilder.toByteArray(jsonObject)
                    sendQueueByte.trySend(byteArray)
                } catch (e: Exception) {
                    Loge.e("调试socket SocketClient heartbeatAndIdleMonitor catch ${e.message}")
                }
            }
    ...
    private fun closeSocketQuietly() {
        Loge.e("调试socket SocketClient closeSocketQuietly $running | ${clientScope.isActive}")
        try {
            socketMutex.tryLock()?.let { locked ->
                if (locked) {
                    try {
                        ...
                        Loge.e("调试socket SocketClient closeSocketQuietly catch1 ${e.message}")
                    } finally {
                        socket = null
                        socketMutex.unlock()
                        Loge.e("调试socket SocketClient closeSocketQuietly finally")
                    }
                }
            }
        } catch (e: Exception) {
            Loge.e("调试socket SocketClient closeSocketQuietly catch2 ${e.message}")
        }
    }
}
