package com.testrecycling.toolsapp.fitsystembar
 ...
 */
class FitSystemBarHelper(private val view: View, private val TAG: String = "FitSystemBarHelper", private val callBack: (orientation: Orientation) -> Boolean) {
    ...
    companion object {
         ...
         */
        fun attachView(view: View, start: Boolean = false, top: Boolean = false, end: Boolean = false, bottom: Boolean = false) = attachView(view) {
            when (it) {
         ...
         * @param bottom 是否需要paddingBottom
         */
        @BindingAdapter(value = ["fitStart", "fitTop", "fitEnd", "fitBottom"], requireAll = false)
        fun View.fitSystemBar(start: Boolean = false, top: Boolean = false, end: Boolean = false, bottom: Boolean = false) {
            attachView(this) {
                when (it) {
                    Orientation.Start -> start
                    Orientation.Top -> top
     ...
     * 针对不同版本处理Insets
     */
    private fun formatInsets(insetsCompat: WindowInsetsCompat, initialPadding: RelativePadding) {
